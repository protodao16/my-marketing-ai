...
//      many Safes were actually created and by how many distinct creators.
//   4. Emits a JSON blob + a ready-to-paste row matching the DPL/Caldera columns
...
//   node safe-check.mjs --chainlist ./rpcs.json --chainId 61900   # resolve RPC from chainlist dump
//   node safe-check.mjs --batch ./leads.json                      # [{name, chainId?, rpcUrl?}, ...]
//
// NOTE: needs outbound access to the target RPC (and to chainlist if --chainlist
// is a URL). In a locked-down egress sandbox those are blocked; run it where the
// network is open, or allowlist the RPC hosts. npm deps install fine either way.
//
...
// Collect the authoritative address set for a version. safe-deployments exposes a
// canonical `defaultAddress` plus per-network overrides (e.g. the eip155 variant);
// we test the default and any network-specific address for this chainId.
function addressesFor(version, chainId) {
  ...
  const add = (label, dep) => {
    ...
    const na = dep.networkAddresses || {};
    const forChain = na[String(chainId)];
...
async function countSafes(url, factories, latest, chunk) {
  // Scan ProxyCreation logs from each deployed factory across the whole chain,
  // in `chunk`-sized block windows. Returns distinct proxies + distinct creators.
  ...
  for (const f of factories) {
    for (let from = 0; from <= latest; from += chunk) {
      ...
      for (const log of logs) {
        // proxy address is in data (1.3.0) or topics[1] (1.4.1+). Grab both defensively.
...
async function resolveRpc(o) {
  if (o.rpc) return o.rpc;
  if (o.chainlist && o.chainId != null) {
    const raw = /^https?:\/\//.test(o.chainlist)
      ? await (await fetch(o.chainlist)).json()
      : JSON.parse(fs.readFileSync(o.chainlist, "utf8"));
    const entry = raw.find((c) => c.chainId === o.chainId);
    if (!entry) throw new Error(`chainId ${o.chainId} not in chainlist dump`);
    const good = (entry.rpc || [])
      .map((r) => (typeof r === "string" ? r : r.url))
      .filter((u) => u && u.startsWith("https") && !u.includes("${"));
    if (!good.length) throw new Error(`no usable https RPC for chainId ${o.chainId}`);
    return good[0];
  }
  throw new Error("provide --rpc, or --chainlist + --chainId");
}

async function run(one) {
  const url = await resolveRpc(one);
  const out = { name: one.name || url, rpc: url };
  try {
    out.chainId = parseInt(await rpc(url, "eth_chainId"), 16);
    out.latestBlock = parseInt(await rpc(url, "eth_blockNumber"), 16);
  } catch (e) {
    out.error = `RPC unreachable: ${e.message}`;
    return out;
  }
  out.code = await checkCode(url, out.chainId);
  const deployedFactories = out.code.filter(
    (c) => c.label.startsWith("Proxy factory") && c.deployed === true
  );
  if (one.logs !== false && deployedFactories.length) {
    out.safes = await countSafes(url, deployedFactories, out.latestBlock, one.chunk || 100000);
  }
  // Build the two sheet cells.
  out.cell_safe_state = out.code
    .map((c) => `${c.label}: ${c.deployed === true ? "YES" : c.deployed === false ? "NO" : c.deployed}`)
    .join("\n") + `\nBy eth_getCode at block ${out.latestBlock}, ${new Date().toISOString().slice(0, 10)}.`;
  out.cell_safes_created = out.safes
    ? `${out.safes.count} distinct Safe proxies` +
      ...
      (out.safes.scanErrors ? ` (${out.safes.scanErrors} log-scan chunks failed — narrow --chunk)` : "")
    : deployedFactories.length
    ? "log scan skipped (--no-logs)"
    : "no proxy factory deployed — nothing to count";
  return out;
}
...
(async () => {
  const o = args();
  let jobs;
  if (o.batch) jobs = JSON.parse(fs.readFileSync(o.batch, "utf8")).map((j) => ({ ...j, chainlist: o.chainlist, chunk: o.chunk, logs: o.logs }));
  else jobs = [o];
